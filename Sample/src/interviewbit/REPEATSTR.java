package interviewbit;

import java.util.HashMap;
import java.util.HashSet;

public class REPEATSTR {
  public int lengthOfLongestSubstring(String input) {
    int max_len = 0;
    int current_len = 0;
    HashMap<Character, Integer> map = new HashMap<>();
    int previous = 0;
    for (int i = 0; i < input.length(); i++) {
      Character c = input.charAt(i);
      if (map.containsKey(c) == false) {
        map.put(c, i);
        current_len++;
        if (current_len > max_len) {
          max_len = current_len;
        }
      } else {
        current_len = i - map.get(c);
        int temp = map.get(c);
        // removeElements(input, map, previous, temp);
        map = addELements(input, map, temp, i);
        previous = temp;
        map.put(c, i);
      }
    }
    return max_len;
  }

  private HashMap<Character, Integer> addELements(String input, HashMap<Character, Integer> map,
      int start, int end) {
    map = new HashMap<>();
    for (int i = start + 1; i <= end; i++) {
      map.put(input.charAt(i), i);
    }
    return map;
  }

  private void removeElements(String input, HashMap<Character, Integer> map, int start, int end) {
    for (int i = start; i <= end; i++) {
      Character key = input.charAt(i);
      if (map.containsKey(key)) {
        map.remove(key);
      }
    }

  }

  public static void main(String[] args) {
    REPEATSTR obj = new REPEATSTR();
    //String input = "abcabcbb";
     //String input = "bbbbbb";
     String input =
     "";
    System.out.println(obj.lengthOfLongestSubstring(input));
  }
}
